...
if (!current) {
  current = sh
    .exec(
      "echo `echo " +
        travisBranch +
        " | cut -d'/' -f 2-`-$(git rev-parse HEAD | cut -c1-7)",
      { silent: true }
    )
    .stdout.replace("\n", "");
}
...
createDeployment()
  ...
  .then(deployment => {
    ...
    sh.cat(compose).exec(`ssh -o "IdentitiesOnly yes" \
      ...
      var logUrl = `https://travis-ci.com/Badmuts/hsleiden-ipsenh-api/builds/${process
        .env.TRAVIS_BUILD_ID}`;
      var envUrl = env == "staging"
        ? `https://staging.api.ipsenh.daan.codes`
        : env == "production"
          ? `https://api.ipsenh.daan.codes`
          : `https://${travisBranch}.api.ipsenh.daan.codes`;
